...
app.get("/campgrounds/:id",function(req, res) {
    ...
    Image.findById(req.params.id).populate('comments').exec(function(error, images) {
                ...
                if(!images.lat )
        {
            ...
            console.log(images.lat) ;  
            var link = "https://api.openweathermap.org/data/2.5/weather?lat=" +images.lat.toString() +"&lon=" + images.long.toString() + "&APPID=4ac7e44c80e2dfa6ebd9ca6237bb075a";
            console.log(link);
            request(link,function(error,response,body){
                    if(!error){
                        
                        var data = JSON.parse(body);
                      res.render("show.ejs",{camp:images,data:data});
